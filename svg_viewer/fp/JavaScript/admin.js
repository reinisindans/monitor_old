...
function get_data_for_tag() {
	...
	var url = baseURL+baseParam+"&tag="+id+"&format=json&include=bib,data&sort=creator";
	...
	$.get(url, null, function(data, status) {
      ...
      console.log(data);
        ...
        $().add(citation).attr("entry", zoterokey).appendTo("#citations");
            ...
            $().add("<p>hi "+note['data']['title']+"</p><p>"+note['data']['url']+"</p>").css("margin-left", "2em").appendTo(element);
...
//document.getElementById("targetC").innerHTML = htmldata;
var theDiv = document.getElementById("targetC");
theDiv.innerHTML = htmldata;
}*/
 //var urlarray = new Array();    //create array of all url to post to php
  var urlarray = [];    //create object of all url to post to php
...
//console.log('BibtexDisplay aufgerufen schritt 24');
 ...
 //	console.log('urlall:'+urlall);
  ...
  this.createTemplate = function(entry, output){ //neu: output
    ...
    do {
      // find next if
      var conds = tpl.find(".if");   //gibt value zurück, der bedingung erfüllt
      
      if (conds.length === 0) {  //wenn value leer = nichts gefunden, stoppe diese Schleife
        break;
      }
      	
      // check if, das einen value hat
      var cond = conds.first(); //.first gibt erstes Element/Wort des conds objekt
      cond.removeClass("if");
      var ifTrue = true;
      var classList = cond.attr('class').split(' ');  //liste der klassen ohne if erzeugt, mit komma getrennt, wo ein leerzeichen ist 
      $.each( classList, function(index, cls){ //iterieren über Liste der Klassen
      	if(cls[0]=="!" &&
    ...
    /*else if (key == "DATE") {							//Ddate macht fehler, moment nicht bekannt
                ...
                var privateName = ID();
                 if (urlallg !== ''){
                 //  urlarray[privateName] = urlallg;
                 urlarray.push(urlallg); 
                  }
             ...
             // var isValid = collectURL(urlallg, urlarray); //array mit url entsteht
               //   console.log(isValid);
...
//////////////End of topic search
  ...
  this.displayBibtex = function(input, output) {
    ...
    b.setInput(input); //hier sind alle annotes noch dabei
...
function createWebPage(defaultTemplate) {
  ...
  $("#check-start").click(function() {   
    //  html='<div>'+Object.values(key)+'</div>';  
   // document.getElementById('cardContainer').innerHTML = html; 
    //urlarray.push("http://www.google.de"); 
    //urlarray.key3 = "http://www.google.de";  
  
  //var urljson= "hallo";
  //Object.assign(urlarray, {key3: "valuetdgr3"});
  console.log(typeof urlarray);
  console.log(urlarray);
  var urljson = JSON.stringify(urlarray); //urljson = string [] leer?;;;;;;urlarray=object mit url inhalten
  //obj = JSON.parse(urljson);
  
  for (var urlkey in urlarray) {
    console.log( urlarray[urlkey]);
  }
  //console.log(urlarray);*/
  /*
  var a = '[{_dheu45: www.google.de},{_2348fh:www.ioer-monitor.de}]';
  console.log(a);
  var json_a = $.parseJSON(JSON.stringify(a));
  console.log(json_a);
  $.post("admin.php",{obj:json_a});
*/
  ...
  //urlarray.push("http://www.google.de"); 
  //urlarray.push("http://www.googlgrgre.de");
  /*var html='';
  $.each( urlarray, function( Object ) {
    console.log(Object);
  $.each( Object, function( key, value ) {
    console.log("1");
    document.getElementById('cardContainer').innerHTML = key + ": " + value ;
  });
  });
  */
 ...
 jsarray[1] = "Volvo";
 jsarray[2] = "BMW";	
      var urljson = JSON.stringify(urlarray);
    console.log("gedrückt");
    $.ajax({
      url: 'urlcheck.php',
      type: 'post',
      data: {'mydataa': urlarray}, 
  ...
  //    cache: false,
      ...
      success: function(){
       // window.location.replace('urlcheck.php'); // window.location.replace('urlcheck.php');
       // location.reload();  //lade admin.php neu um post zu empfangen
          console.log("OK");
          console.log(jsarray);
          console.log(urlarray);
          console.log(urljson); //scheint nicht
...
/*
   ...
   function friends(){
    ...
    x.innerHTML = '<?php include_once "urlcheck.php"; echo("ist da");';
    }      
   friends();    */
