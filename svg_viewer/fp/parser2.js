  ...
  this.hideEntry = function(word) {
   ...
   if (container.first().hasClass("bibtexentry")) {      
      ...
      container.each( 
        function () {
          var shouldHide = true;          
        //  var shouldHeadHide = true;
          $(this).find(".bibtexentry:visible").each( 
            function() {
              if(!funcCaller.checkEntry($(this),word)){ //wenn eintrag das Wort (@autor=Adrian, oder nur bloßer Freitextbegriff) nicht enthält, verstecke es
...
function loadExtras() { 
  ...
  $(".bibtex_author").each(function(i, obj) {
    ...
    $(".bibtex_keywords").each(function(i, obj) {
  	keywordsList($(this));
  });
