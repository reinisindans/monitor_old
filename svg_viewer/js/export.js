...
$(function() {
	$("#kartenlink").click(function(){
	    ...
	    var parameters = urlparamter.getAllUrlParameters();

		$.ajax({
			type: 'GET',
			url:  'php/Kartenlink.php',
			data: {
                ...
                pdf_dialog_container.dialog({
                    ...
                    open: function () {
                        ...
                        if (raeumlicheauswahl.getRaeumlicheGliederung() === 'gebiete') {
                            ...
                            cloned_layer = cloneLayer(getRasterLayer());
                        }
                        cloned_layer.addTo(print_map);
                        try {
                            overlay = cloneLayer(zusatzlayer.getLayerSet());
                            overlay.addTo(print_map);
                            overlay.setStyle(zusatzlayer.getStyleSet());
                        }catch(err){}

                        //disable click events for the layer
                        $('#print_map svg').click(false);
                        $('#print_map svg path.leaflet-interactive ').css('pointer-events', 'none');
                        print_map.off('click', OnClickRaster);
                        ...
                        function createExport() {
                            ...
                            let opt = {
                                margin: 5,
                                quality: 1,
                                filename: indikatorauswahl.getSelectedIndikator() + "_" + getRaumgliederungID() + "_" + zeitslider.getTimeSet() + '.' + format,
                        ...
                        function setSVGContent() {
                            ...
                            let histogramm_print = $('#print_histogramm');
                            //resize and append DOM Data for pdf
...
function saveIMAGE(dataBlob, filesize) {
    ...
    saveAs(dataBlob, indikatorauswahl.getSelectedIndikator()+"_"+getRaumgliederungID()+"_"+zeitslider.getTimeSet() +'.png');
}
function savePDF(dataBlob, filesize) {
    ...
    doc.save(indikatorauswahl.getSelectedIndikator()+"_"+getRaumgliederungID()+"_"+zeitslider.getTimeSet() + ".pdf");
}
//**dataURL to blob**
