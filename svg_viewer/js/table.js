const table = {
    ...
    init:function(){
        const tableObject = this.getTableBodyObject(),
            table = this;
        ...
        $('.indikatoren_diagramm_ags')
            ...
            .click(function() {
                ...
                let name = $(this).data('name');
                let ind = $(this).data('ind');
                let wert = $(this).data('wert');
                openStatistik(ags,name,wert,ind,true);
            });
            ...
            .mouseenter(function() {
                $(this).delegate('tr', 'mouseover mouseleave', function (e) {
                    if (e.type === 'mouseover') {
                        ...
                        ags.trim();
                        indicatorJSONGroup.highlight(ags,false);
                    }
                    else {
                        $(this).removeClass("hover");
                        indicatorJSONGroup.resetHightlight();
                    }
                });
                $(this).delegate('tr', 'click', function (e) {
                    if (e.type === 'click') {
                        ...
                        let ags = $(this).find('.td_ags').text();
                        ags.trim();
                        indicatorJSONGroup.highlight(ags,true);
                    }
                    else {
                        $(this).removeClass("hover");
                        indicatorJSONGroup.resetHightlight();
    ...
    create:function(){
        ...
        let layer_array = _.sortBy(getMapLayerArray(),"gen");
        ...
        function createTableHeader(){
            ...
            var html = '<thead id="thead" class="full-width">'+
                ...
                '<th colspan="'+colspan+'" data-sorter="false" class="sorter-false expand" id="header_ind_set">'+indikatorauswahl.getSelectedIndikatorText_Lang()+' ('+zeitslider.getTimeSet()+')</th>'+
                '</tr>'+
        ...
        function createTableBody() {
            ...
            $.each(layer_array,function(key,value){
                ...
                //set the variables
                let ags = value.ags;
                let grundakt_value = value.grundakt;
                let wert = '<b>'+value.value_comma+'</b>';
                let value_int = value.value;
                let fc = value.fc;
                let des= value.des;
                //hc
                let hc = value.hc;
                let name = value.gen;
                let raumgl = getRaumgliederungID();
                if(typeof getRaumgliederungfeinID()!=='undefined'){raumgl= getRaumgliederungfeinID();}
                ...
                let img_trend = '<img data-name="' + value.gen + '" data-ags="' + ags + '" data-ind="' + ind + '" data-wert="' + value_int + '" data-einheit="' + einheit + '" title="Veränderung der Indikatorwerte für die Gebietseinheit" class="indsingle_entwicklungsdiagr" id="indikatoren_diagramm_ags' + ags + '" src="images/icon/indikatoren_diagr.png"/>';
                let img_trend_ind = '<img data-name="' + value.gen + '" data-ags="' + ags + '" data-ind="' + ind + '" data-wert="' + value_int + '" data-einheit="' + einheit + '" title="Veränderung des Indikatorwertes für die Gebietseinheit" class="ind_entwicklungsdiagr" id="indikatoren_diagramm_ags_ind' + ags + '" src="images/icon/indikatoren_verlauf.png"/>';
    ...
    expand:function(){
        ...
        function defCalls(){
            ...
            $.each(expand_array,function(key,value){
                requests.push(getIndicatorValueByMapAGS(value));
            });
        ...
        defCalls().done(function(arr) {
            ...
            if(expand_array.length===1){
                results.push(JSON.parse(arr[0]));
            }else{
                for(let i=0;i<=expand_array.length-1;i++){
                    results.push(JSON.parse(arr[i][0]));
                }
            }
            //sort by count
            ...
            $.each(results,function(key,values_expand){
                ...
                if(count==10){
                    ...
                    $.when(getIndicatorValueByMapAGS(obj_brd,obj_ags)).done(function(data){
                    ...
                    if(typeof getRaumgliederungfeinID()!=='undefined'){
                        ...
                        $.each(selection,function(key,value){
                            ...
                            $.when(getIndicatorValueByMapAGS(obj_brd,obj_ags_bld)).done(function(data){
                                let data_array_bld = JSON.parse(data);
                ...
                //expand with BRD ord BLD as new columns outside the table
                    ...
                    second_header_row.append('<th class="'+grey_border+' '+class_expand+' sorter-false expand header">'+header_text_first_row+')</th><th class="'+class_expand+'">'+header_text_second_row+')</th>');
                ...
                //time shift expand
                    ...
                    if(table_expand_panel.getDifferenceState()){
                        x=x+1;
                        td_diff ='<th class="'+class_expand+' header">Differenz ('+time_set+' bis '+zeitslider.getTimeSet()+')</th>';
                    }
                    //append the header
...
const table_expand_panel = {
    ...
    fill: function(){
        ...
        panel.clear();
        //DDM Indikatoren
        cloneIndicatorMenu('kat_auswahl_table','link_kat_table','left',false,true);
        ...
        if(jahreArray.length > 1 || indikatorauswahl.getFilteredPossibleYears()[0] > parseInt(zeitslider.getTimeSet())) {
        ...
        else if(value.toString().indexOf("k") >= 0 ){
    ...
    init:function(){
        ...
        panel.getOpenButtonObject().unbind().click(function(){
            panel.open();
        });
        //panel button to load the user choice and expand the table
        panel.getButtonLoadExpandObject().unbind().click(function(){
            panel.removeExpandColumns();
            panel.close();
            if(panel.expandArray.length >0) {
                try {
                    table.expand();
                } catch (err) {
                    alertError();
        ...
        //reset the expand
        panel.getButtonClearExpandObject()
            .unbind()
            .click(function(){
                panel.setExpandArray([]);
                panel.removeExpandColumns();
        ...
        this.getIndikatorauswahlDDMObject()
            ...
            .dropdown({
                ...
                onAdd: function (addedValue, addedText, $addedChoice) {
                    panel.expandArray.push({id:addedValue,text:addedText,time:zeitslider.getTimeSet(),einheit:false, count: 50});
                    //sort the time array desc
                    panel.expandArray = _.sortBy(panel.expandArray, 'total').reverse();
                    //disable other choice possibilities
                    panel.getHinweisOnlyOlderTimeShiftsObject().hide();
                    panel.getTrendAuswahlContainer().hide();
                    panel.getUebergeordneteKenngroessenObject().hide();
                    //clear ddm kenngroessen bld ord brd
                    panel.getKenngroessenauswahlDDMObject().find('.label').each(function () {
        ...
        this.getZeitschnittauswahlDDMObject()
            .unbind()
            .dropdown({
                onAdd: function (addedValue, addedText, $addedChoice) {
                    panel.expandArray.push({
                ...
                onLabelRemove: function (value) {
                    ...
                    let selection = panel.getZeitschnittauswahlDDMObject().dropdown('get value').split(',');
        ...
        this.getKenngroessenauswahlDDMObject()
            ...
            onAdd: function (addedValue, addedText, $addedChoice) {
                if(addedValue === 'brd'){
                    panel.expandArray.push({id:addedValue,text:'Gesamte Bundesrepublik ('+zeitslider.getTimeSet()+')',time:zeitslider.getTimeSet(),einheit:false, count: 15});
                }
                else if(addedValue === 'bld'){
                    panel.expandArray.push({id:addedValue,text:'Übergeordnetes Bundesland ('+zeitslider.getTimeSet()+')',time:zeitslider.getTimeSet(),einheit:false,count: 15});
                }
                else{
                    panel.expandArray.push({id:addedValue,text:addedText,time:zeitslider.getTimeSet(),einheit:false,count: 10});
                }
                $(this).blur();
            },
            onLabelRemove: function (value) {
...
const table_filter_panel = {
    ...
    init:function(){
        const panel = this;
        panel.fill();
        //bin the choeckbox interactions
        panel.getFilterCheckBoxObject().checkbox({
            onChecked:function(){
                panel.showDes($(this).data('value'));
                setTimeout(function(){table.setRang();},500);
                panel.close();
            },
            onUnchecked: function() {
                panel.removeDes($(this).data('value'));
                setTimeout(function(){table.setRang();},500);
                panel.close();
            }
        });
        ...
        panel.getCloseIconObject().unbind().click(function(){
            panel.close();
        });
    },
    fill:function(){
        let not_showing=["ror","lks","kfs","g50","stt"],
            raumgl = getRaumgliederungID();

        if(getRaumgliederungfeinID()){
            raumgl = getRaumgliederungfeinID();
        }
        ...
        this.getButtonDomObject()
            ...
            .click(function(e){
                ...
                table.getDOMObject().TableCSVExport({
                    ...
                    delivery: 'download',
                    separator: ';',
                    filename:indikatorauswahl.getSelectedIndikator()+"_"+getRaumgliederungID()+"_"+zeitslider.getTimeSet()+".csv"
                });
                table.setStickTableHeader();
            });
    }
